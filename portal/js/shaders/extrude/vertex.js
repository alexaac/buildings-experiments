...
const extrudeVertexShader = `
  ...
  void main() {
    ...
    // v_pos = -(u_model_view_matrix * pos).z;
    
    // vec2 ndcPos = gl_Position.xyz / gl_Position.w;
    // point_pos = u_resolution * (ndcPos*0.5 + 0.5);

    // vec3 ndc = gl_Position.xyz / gl_Position.w; //perspective divide/normalize
    // vec2 viewportCoord = ndc.xy * 0.5 + 0.5; //ndc is -1 to 1 in GL. scale for 0 to 1
    // point_pos = viewportCoord * u_resolution;
    point_pos = vec2(0.0, 0.0);
